...
export default function AuthForm({ mode }) {
  ...
  async function submit(event) {
    ...
    setLoading(false);
    if (response.ok) { window.localStorage.setItem("refactorflow-email", email); if (data.session?.access_token) window.localStorage.setItem("refactorflow-access-token", data.session.access_token); if (mode === "signin") { window.localStorage.setItem("refactorflow-show-name-prompt", "1"); router.push("/dashboard"); return; } }

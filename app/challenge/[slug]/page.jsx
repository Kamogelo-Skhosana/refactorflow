...
export default async function ChallengeDetailPage({ params }) {
  ...
  return <main className="challenge-detail"><nav className="dashboard-nav shell"><Link className="brand" href="/">Refactor<span>Flow</span></Link><div className="dashboard-nav-links"><Link href="/challenge">All challenges</Link><Link href="/dashboard">Dashboard</Link><Link href="/settings">Settings</Link></div></nav><section className="challenge-detail-heading shell"><Link className="back-link" href="/challenge">&larr; Python fundamentals</Link><div className="challenge-detail-meta"><span className="kicker">{challenge.language} Â· {challenge.difficulty}</span><span className="challenge-test-count">Practice challenge</span></div><h1>{challenge.title}</h1><p>{challenge.description}</p><div className="challenge-tip"><strong>Take your time.</strong> Read the problem, form a hypothesis, and make the smallest useful change.</div></section><section className="challenge-editor-section shell"><div className="challenge-statement"><p className="kicker">Your task</p><h2>Write a solution that passes the challenge.</h2><p>Use the starter code below as your workspace. When you are ready, run your solution to receive feedback.</p><p className="hidden-tests-note">Your solution will be checked against private test cases.</p></div><ChallengeEditor starterCode={challenge.starter_code} slug={challenge.slug} /></section></main>;
}
